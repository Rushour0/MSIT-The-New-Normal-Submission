    ...
    def handleNotification(self, hnd, data):
        ...
        # The fetch characteristic controls the communication with the activity characteristic.
            ...
            if data[:3] == b"\x10\x01\x01":
                ...
                if self.device.last_timestamp > self.device.end_timestamp - timedelta(
                    minutes=1
                ):
...
class miband(Peripheral):
    ...
    def __init__(self, mac_address, key=None, timeout=0.5, debug=False):
        ...
        self._char_heart_measure = self.svc_heart.getCharacteristics(
            UUIDS.CHARACTERISTIC_HEART_RATE_MEASURE
        )[0]
        ...
        self._char_chunked = self.svc_1.getCharacteristics(
            UUIDS.CHARACTERISTIC_CHUNKED_TRANSFER
        )[0]
        self._char_music_notif = self.svc_1.getCharacteristics(
            UUIDS.CHARACTERISTIC_MUSIC_NOTIFICATION
        )[0]
    ...
    def writeChunked(self, type, data):
        ...
        while remaining > 0:
            ...
            chunk += data[
                (count * MAX_CHUNKLENGTH) : (count * MAX_CHUNKLENGTH) + copybytes
            ]
            count += 1
    ...
    def setAlarm(self, hour, minute, days=(), enabled=True, snooze=True, alarm_id=0):
